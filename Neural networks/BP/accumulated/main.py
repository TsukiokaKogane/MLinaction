...
class Net(object):
    ...
    def forward(self, x, y):
        ...
        b = np.array([[0.0] for i in range(self.q)])
...
if __name__ == '__main__':
    ...
    q = 10
    ll = 3
    net = Net(d=d, q=q, ll=ll, eta=0.01)
    while cnt < iteration:
        err = 0.0

        ng = np.array([0.0 for i in range(ll)])
        ne = np.array([[0.0] for i in range(q)])
        nb = np.array([[0.0] for i in range(q)])
        ...
        for i in range(train_n):
            net.backward(ng, ne, nb, train_data[i])
